...
public class ResourceRequestAllocationService {
	...
	public GetResourcesByUserNameResponse getResourcesByUserName(String userName) throws BusinessException, ParseException {
		...
		for(ResourceRequestAllocation rra : userResources) {
			...
			response.setTotalCost(response.getCpuCost()+response.getDiskCost()+response.getRamCost());
			
			ResourceRequestAllocationResponse rraResp = new ResourceRequestAllocationResponse();
				
			rraResp.setResourceRType(rra.getResourceType());
			rraResp.setOs(rra.getOs());
			rraResp.setCpu(rra.getCpu());
			rraResp.setRam(rra.getRam());
			rraResp.setStorage(rra.getStorage());
